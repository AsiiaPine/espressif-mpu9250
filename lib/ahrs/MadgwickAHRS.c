...
// {
//   Quaternion quaternion = Quaternion_default;
//   state->q = quaternion;
//   return state;
// }
...
Madgwick_state *MadgwickAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz, Madgwick_state *state)
{
  ...
  {
    state = MadgwickAHRSupdateIMU(gx, gy, gz, ax, ay, az, state);
    return state;
  }
  float q0 = state->q->q0;
  float q1 = state->q->q1;
 ...
 */
void MadgwickGetVector(float *angle, float *x, float *y, float *z, Madgwick_state *state)
{
  float ang = 2.0 * acos(state->q->q0);
  float sin_angle = sin(ang / 2.0);
  *angle = ang;
  *x = state->q->q1 / sin_angle;
  *y = state->q->q2 / sin_angle;
  *z = state->q->q3 / sin_angle;
};
 ...
 */
void MadgwickGetEulerAngles(float *heading, float *pitch, float *roll, Madgwick_state *state)
{
...
{
  MadgwickGetEulerAngles(heading, pitch, roll, state);
