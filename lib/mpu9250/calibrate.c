...
static void init_imu(int i2c_port, int i2c_master_sda, int i2c_master_scl, state_container* mag_state, int i2c_addr)
{
  static bool init_imu_done = false;
  if (init_imu_done)
    return;
  i2c_mpu9250_init(&cal, i2c_port, i2c_master_sda, i2c_master_scl, mag_state, i2c_addr);
  init_imu_done = true;
}
...
{
  init_imu(i2c_port, i2c_master_sda, i2c_master_scl, state, i2c_addr);
  ...
  for (int i = 0; i < NUM_GYRO_READS; i += 1)
  {
    ...
    ESP_ERROR_CHECK(get_gyro(&vg, i2c_port, i2c_addr));
 ...
 */
void calibrate_accel_axis(int axis, int dir, int i2c_port, int i2c_addr)
{
  vector_t va;
  ...
  {
    get_accel(&va, i2c_port, i2c_addr);
 ...
 */
void run_next_capture(int axis, int dir, int i2c_port, int i2c_addr)
{
  ESP_LOGW(TAG, "Point the %s axis arrow %s.", axes[axis], directions[dir]);

  wait();
  calibrate_accel_axis(axis, dir,i2c_port, i2c_addr);
}

void calibrate_accel(int i2c_port, int i2c_master_sda, int i2c_master_scl, state_container* mag_state, int i2c_addr)
{
  init_imu(i2c_port, i2c_master_sda, i2c_master_scl, mag_state, i2c_addr);

  ESP_LOGI(TAG, "--- ACCEL CALIBRATION ---");

  run_next_capture(X_AXIS, DIR_UP, i2c_port, i2c_addr);
  run_next_capture(X_AXIS, DIR_DOWN, i2c_port, i2c_addr);
  run_next_capture(Y_AXIS, DIR_UP, i2c_port, i2c_addr);
  run_next_capture(Y_AXIS, DIR_DOWN,i2c_port, i2c_addr);
  run_next_capture(Z_AXIS, DIR_UP, i2c_port, i2c_addr);
  run_next_capture(Z_AXIS, DIR_DOWN, i2c_port, i2c_addr);
...
void calibrate_mag(int i2c_port, int i2c_master_sda, int i2c_master_scl, state_container* mag_state, int i2c_addr)
{
