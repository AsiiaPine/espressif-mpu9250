...
esp_err_t ak8963_init(i2c_port_t i2c_number, vector_t *mag_offset, vector_t *mag_scale)
{
  if (initialised)
  {
  ...
  state_container state = {.i2c_num=i2c_number, .mag_offset=mag_offset, .mag_scale=mag_scale};

  // connection with magnetometer
  uint8_t id;
  ak8963_get_device_id(&id, state.i2c_num);

  if (id & AK8963_WHO_AM_I_RESPONSE)
  {
    ak8963_get_sensitivity_adjustment_values(&state);
    vTaskDelay(10 / portTICK_RATE_MS);
    ak8963_set_cntl(AK8963_CNTL_MODE_CONTINUE_MEASURE_2, state.i2c_num);
    initialised = true;
    return ESP_OK;
  }
  else
...
esp_err_t ak8963_get_data_ready(bool *val, uint8_t i2c_num )
{
  uint8_t bit;
...
esp_err_t ak8963_get_device_id(uint8_t *val, uint8_t i2c_num)
{
  return i2c_read_byte(i2c_num, AK8963_ADDRESS, AK8963_WHO_AM_I, val);
}

esp_err_t ak8963_get_sensitivity_adjustment_values(state_container *state)
{
  uint8_t i2c_num = state->i2c_num;
  vector_t asa = state->asa;
  esp_err_t ret;

  // Need to set to Fuse mode to get valid values from this.
  uint8_t current_mode;
  ret = ak8963_get_cntl(&current_mode, i2c_num);
  if (ret != ESP_OK)
    return ret;

  ret = ak8963_set_cntl(AK8963_CNTL_MODE_FUSE_ROM_ACCESS, i2c_num);
  if (ret != ESP_OK)
    return ret;
  ...
  return ak8963_set_cntl(current_mode, i2c_num);
}

esp_err_t ak8963_get_mag(vector_t *v, state_container *state)
{

  uint8_t i2c_num = state->i2c_num;
  vector_t* mag_scale = state->mag_scale;
  vector_t* mag_offset = state->mag_offset;

  esp_err_t ret;
  uint8_t bytes[6];

  ret = ak8963_get_mag_raw(bytes, i2c_num);
  if (ret != ESP_OK)
  {
    return ret;
  }
  ...
  v->x = (xi * state->asa.x - mag_offset->x) * mag_scale->x;
  v->y = (yi * state->asa.y - mag_offset->y) * mag_scale->y;
  v->z = (zi * state->asa.z - mag_offset->z) * mag_scale->z;
  // ESP_LOGW(TAG, "mag     -> %0.4f %0.4f %0.4f", v->x, v->y, v->z);

  return ESP_OK;
}

esp_err_t ak8963_get_mag_raw(uint8_t bytes[6], int i2c_num)
{
...
esp_err_t ak8963_get_cntl(uint8_t *mode, uint8_t i2c_num)
{
  return i2c_read_byte(i2c_num, AK8963_ADDRESS, AK8963_CNTL, mode);
}

esp_err_t ak8963_set_cntl(uint8_t mode, uint8_t i2c_num)
{
  return i2c_write_byte(i2c_num, AK8963_ADDRESS, AK8963_CNTL, mode);
}

void ak8963_print_settings(state_container *state)
{
  uint8_t i2c_num = state->i2c_num;
  vector_t* mag_scale = state->mag_scale;
  vector_t* mag_offset = state->mag_offset;  vector_t asa = state->asa;
  ...
  uint8_t device_id;
  ESP_ERROR_CHECK(ak8963_get_device_id(&device_id, state->i2c_num));

  uint8_t cntl;
  ESP_ERROR_CHECK(ak8963_get_cntl(&cntl, state->i2c_num));
  ...
  ESP_LOGI(TAG, "--> Offset:");
  ESP_LOGI(TAG, "  --> x: %f", mag_offset->x);
  ESP_LOGI(TAG, "  --> y: %f", mag_offset->y);
  ESP_LOGI(TAG, "  --> z: %f", mag_offset->z);
  ESP_LOGI(TAG, "--> Scale:");
  ESP_LOGI(TAG, "  --> x: %f", mag_scale->x);
  ESP_LOGI(TAG, "  --> y: %f", mag_scale->y);
  ESP_LOGI(TAG, "  --> z: %f", mag_scale->z);
}
