...
static const char *TAG = "i2c-easy";
 ...
 */
esp_err_t i2c_master_init(i2c_port_t i2c_num, uint8_t gpio_sda, uint8_t gpio_scl)
{
...
esp_err_t i2c_read_bytes(i2c_port_t i2c_num, uint8_t periph_address, uint8_t reg_address, uint8_t *data, size_t data_len)
{
  ...
  ret = i2c_master_cmd_begin(i2c_num, cmd, 1000 / portTICK_RATE_MS);
  // ESP_LOGI(TAG, "ESP_OK i2c_read_bytes:%d", ret==ESP_OK);
  ...
  i2c_master_stop(cmd);
  ret = i2c_master_cmd_begin(i2c_num, cmd, 1000 / portTICK_RATE_MS);
  // ESP_LOGI(TAG, "ESP_OK i2c_read_bytes 1:%d", ret==ESP_OK);
...
void i2c_reset_rx_tx_fifo(i2c_port_t i2c_num, uint8_t gpio_sda, uint8_t gpio_scl){
